<?php
  ...
  if (isset($_GET['del_id'])) { //проверяем, есть ли переменная
    ...
    $flow = mysqli_query($link, "SELECT `book_link` FROM `Books` WHERE `id` = {$_GET['del_id']}");
//echo '<p>Произошла ошибка: ' . $flow['book_link'] . '</p>';
$row = mysqli_fetch_array($flow);
echo $row['book_link'];

echo  gettype($row['book_link']) ;
wp_delete_file($row['book_link']);

    $sql = mysqli_query($link, "DELETE FROM `Books` WHERE `id` = {$_GET['del_id']}");


$flow = '/home/dep805/new.dep805.ru/docs/wp-content/uploads/product-images1/function.txt';
