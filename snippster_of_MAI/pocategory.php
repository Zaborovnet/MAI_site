...
<form method="post" enctype="multipart/form-data">
    <h6><p>Выберите категорию для поиска:
<select name="Search" style="height:60px !important">
<option></option>
  <option>Для абитуриентов</option>
  <option>Монография</option>
  <option>Тезисы конференции</option>
  <option>Учебно-методическое пособие</option>
...
$selectSearch = $_REQUEST['Search'];
                $query = "SELECT * FROM `Books` WHERE `category` = '$selectSearch' AND  id!=0";
                $cursor = mysqli_query($connection, $query);
