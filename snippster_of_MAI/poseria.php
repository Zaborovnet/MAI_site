...
<form method="post" enctype="multipart/form-data">
    <h6><p>Выберите серию для поиска:

<select name="Search" style="height:60px !important">
 <option></option>
    <option>Прикладная математика для ВТУЗов</option>
  <option>Прикладная математика в примерах и задачах</option>
  <option>Новая университетская библиотека</option>
  <option>Применение систем компьютерной математики и визуального моделирования</option>
  <option>Пособие для самостоятельной работы студентов ИНЖЭКИНа</option>
  <option>Учебно-методические комплексы кафедры математической кибернетики</option>
  <option>Бакалавриат</option>
  <option>Менеджмент для инженера</option>
  <option>Научная мысль</option>
  <option>Textbook for Bachelor os Science Students</option>
...
$selectSearch = $_REQUEST['Search'];
                $query = "SELECT * FROM `Books` WHERE `seria` = '$selectSearch' AND  id!=0";
                $cursor = mysqli_query($connection, $query);
